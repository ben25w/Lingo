(function () {
  ...
  // Elements (some may be missing; we guard later)
  var gridEl        = qs('#grid');
  ...
  var modeSel       = qs('#mode');          // single/two (two-player later)
  var lenSel        = qs('#wordLength');    // 4/5/6
  var guessesInput  = qs('#guesses');       // attempts
  var revealSel     = qs('#reveal');        // first/last/none
  var timerSel      = qs('#timerSel');      // optional seconds picker
  var singlishToggle= qs('#singlishToggle');// optional checkbox
  ...
  var state = {
    ...
    // Singlish (5 letters)
    singlish5: [],
    // Active dictionary (not enforced for invalid guesses now, kept for future)
    dictCurrent: {},
  ...
  function loadWords() {
    ...
    // Singlish (5 letters)
    var pSing = fetch('/words/SINGLISH').then(function(r){
      if (!r.ok) return '';
      return r.text();
    }).then(function(txt){
      if (!txt) return;
  ...
  function pickSecret(wordSet, len) {
    var pool, usedMap;
    if (wordSet === 'singlish') {
      pool   = state.singlish5;
      usedMap= state.used.sg5;
    } else {
      pool   = state.wordsByLen[len] || [];
      usedMap= state.used[len];
    }
    if (!pool || pool.length === 0) return '';
    if (Object.keys(usedMap).length >= pool.length) {
      // reset repeats
      state.used.sg5 = wordSet==='singlish' ? {} : state.used.sg5;
      if (wordSet !== 'singlish') state.used[len] = {};
  ...
  function firstEditableCol(len, lockedIdx) {
    var locked = {};
    for (var i=0;i<lockedIdx.length;i++) locked[lockedIdx[i]] = true;
  ...
  function buildCurrentDict() {
    ...
    if (state.settings.wordSet === 'singlish') {
      list = state.singlish5;
    } else {
      list = state.wordsByLen[state.settings.len] || [];
    }
    for (var i=0;i<list.length;i++) state.dictCurrent[list[i]] = true;
  }
  ...
  function startNewRound() {
    ...
    // Singlish toggle overrides wordSet
    ...
    if (guessesInput && guessesInput.value) {
      var g = parseInt(guessesInput.value, 10);
      if (isNaN(g)) g = 6;
    ...
    if (timerSel && timerSel.value) {
      var t = parseInt(timerSel.value, 10);
      if (!isNaN(t) && t > 0) state.settings.timer = t;
    }
    persist();
    ...
    if (!state.round.secret) {
      setMessage('No words loaded. Check /words files.', true);
      return;
    }
    ...
    // Pre-fill locked letters on every row (keeps column fixed)
  ...
  // -------- Typing & submission (physical keyboard only) --------
  document.addEventListener('keydown', function(e){
    ...
    if (/^[a-zA-Z]$/.test(k)) {
      ...
      if (!state.timerRunning) startTimerForRow(); // backspace can start timer too
      moveColBack();
      if (isLocked(state.round.col)) {
        moveColForward(); // cannot delete locked letter
      } else {
  ...
  // -------- Wordle colouring with duplicate handling --------
  function colourRow(guess, secret, rowTiles) {
    ...
    for (i=0;i<len;i++) marks[i] = 'absent';
  ...
  function submitGuess(rawGuess) {
    ...
    // Always colour and consume the row
    ...
    stopTimer(); // wait for first key on next row
    timerEl.style.width = '100%';
    setMessage('');
  }
