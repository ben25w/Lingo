...
// Setup round
state.round.maxRows = state.settings.mode === 'two' ? state.settings.guesses : state.settings.guesses;
...
// If nextBtn was visible, we just finished a round, so toggle starter.
if (!nextBtn.hidden) state.round.starter = state.round.starter === 1 ? 2 : 1;
} else {
